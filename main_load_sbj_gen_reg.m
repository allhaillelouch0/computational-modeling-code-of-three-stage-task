...
t = clock;
% subject list
LIST_SBJ={'YuRongkun', 'ZhengJinrong'};
...
maxd=1; % seed parameter 몇개 사용해 볼 것인지.
exptime=0;
timedu=0;
exp1=clock;
exp0=clock;
...
% Data preparation
for i = 1 : maxi
    Data_pre{i}= load([pwd '\result_save_ruan\' list_sbj{i} '_pre_1.mat']);
    
    tt = dir([pwd '\result_save_ruan']);
    tt = {tt.name};
    maxsess = sum(cell2mat(strfind(tt,[list_sbj{i} '_fmri_']))) - 1;
    
    for ii = 1 : maxsess
        Data_main{i,ii}=load([pwd '\result_save_ruan\' list_sbj{i} '_fmri_' num2str(ii) '.mat']);
    end
end
...
for d= 1 : maxd
    ...
    for i = 1 : 1 : maxi
        ...
        % if you have multiple SBJ files (multiple seed parameters) then
        % you have to find the best one. let's say removed other files but
        % only have the best one.
        
        fname = dir(['result_save_ruan/' 'SBJ_' list_sbj{i} '_*.mat']);
        load([fname.folder '/' fname.name])
        
        data_in{1,1}.model_BayesArb = SBJ.model_BayesArb;
        
        mode.out=99;
        SBJ = eval_ArbitrationRL3c(data_in{1,1}.model_BayesArb.param,data_in,mode);
        
        t = clock;
        save(['result_save_ruan/' 'SBJ_' list_sbj{i} '_' num2str(t(1)), num2str(t(2)), num2str(t(3)),num2str(t(4)) '_regressor.mat'],'SBJ')
